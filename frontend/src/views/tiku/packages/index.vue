    ...
    <el-row :gutter="16">
      <el-col v-for="item in packages" :key="item.id" :xs="24" :sm="12" :lg="8" class="package-col">
        <el-card shadow="hover" class="package-card">
          <div class="package-head"><span class="package-name">{{ item.name }}</span><el-tag>{{ typeText(item.type) }}</el-tag></div>
...
const buy = async (item: PackageItem) => {
  ...
  try {
    const response = await createOrder({ packageId: item.id, provider: "epay" });
