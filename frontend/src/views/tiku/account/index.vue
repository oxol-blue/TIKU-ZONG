<template>
  <div class="tiku-page">
    <el-card shadow="never" class="tiku-card">
      ...
      <div class="actions"><el-button type="primary" :loading="creating" :disabled="!!keyView" @click="create">创建 API Key</el-button><el-button :disabled="!keyView" @click="load">刷新状态</el-button></div>
    ...
    <el-card shadow="never" class="tiku-card">
      ...
      <el-button type="primary" :loading="loadingConfig" :disabled="!keyView" @click="generateConfig">生成配置</el-button>
      <el-input v-if="ocsConfig" v-model="ocsConfig" class="config-input" type="textarea" :rows="12" readonly />
      <el-button v-if="ocsConfig" @click="copyConfig">复制配置</el-button>
    </el-card>
  </div>
</template>
...
import { createApiKey, getApiKey, type ApiKeyView } from "@/api/tiku";
...
const create = async () => { creating.value = true; try { const response = await createApiKey(); plainKey.value = response.data.key; keyView.value = response.data.info; } finally { creating.value = false; } };
